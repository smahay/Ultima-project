...
tcb *scheduler::create_task(const string &task_name, WINDOW *task_win)
{
    ...
    task_table[idx].memory_handle = -1;
    task_table[idx].memory_requested = 0;
...
void scheduler::dump(int level)
{
    ...
    for (int i = 0; i < next_available_task_id; i++)
    {
        ...
        {
            snprintf(buff, sizeof(buff),
                     " [work=%d mem_handle=%d mem_requested=%d]",
                     task_table[i].work_counter,
                     task_table[i].memory_handle,
                     task_table[i].memory_requested);
            out += buff;
        }
